...
local L = {
	EN = {
		...
		trialDesc="Auto farm tower trial",
		trialStatus="STATUS", trialTask="TASK", trialProgress="PROGRESS",
		trialOff="Disabled",
		trialGoTower="Going to tower...",
		trialStarting="Starting Trial...",
		trialNotFound="Trial not found, waiting...",
		trialFarming="Farming ",
		trialDeliver="Delivering to tower...",
		trialDone="✓ Done!",
		trialRestart="Restarting...",
		trialNoItems="No ",
		trialNoItemsSuffix=", waiting...",
		trialRespawn="Respawn → ",
		trialFlyTo="Flying to ",
		trialGoTo="Going to ",
		trialTaking="Taking ",
		trialGone="Gone, next (",
		trialUnits=" pcs",
		trialTaskChanged="Task changed: ",
	},
	RU = {
		...
		trialDesc="Авто фарм триала башни",
		trialStatus="СТАТУС", trialTask="ЗАДАНИЕ", trialProgress="ПРОГРЕСС",
		trialOff="Выключен",
		trialGoTower="Иду к башне...",
		trialStarting="Начинаю Trial...",
		trialNotFound="Триал не найден, жду...",
		trialFarming="Фармлю ",
		trialDeliver="Несу к башне...",
		trialDone="✓ Готово!",
		trialRestart="Рестарт...",
		trialNoItems="Нет ",
		trialNoItemsSuffix=", жду...",
		trialRespawn="Респаун → ",
		trialFlyTo="Лечу к ",
		trialGoTo="Иду к ",
		trialTaking="Беру ",
		trialGone="Пропал, след. (",
		trialUnits=" шт",
		trialTaskChanged="Задание сменилось: ",
	}
}
...
local function EnableNoclip()
	...
	-- Primary: Stepped connection (covers all body parts + accessories)
		...
		for _, p in ipairs(chr:GetDescendants()) do
			if p:IsA("BasePart") then
				pcall(function()
					...
					if rawset then rawset(p, "CanCollide", false) end
				end)
			end
		end
		-- also zero out velocity to prevent physics pushback
		local hrp2 = chr:FindFirstChild("HumanoidRootPart")
		if hrp2 then pcall(function() hrp2.AssemblyLinearVelocity = Vector3.zero end) end
	end)
	...
	if val then
		...
		task.spawn(function()
			...
			while AutoFarmEnabled do
				...
				-- FlyTo: same logic as Trial
				local function FlyTo(hrp, toPos)
					local sPos = hrp.Position
					local d2d = (Vector3.new(toPos.X,0,toPos.Z)-Vector3.new(sPos.X,0,sPos.Z)).Magnitude
					local steps = math.max(5, math.floor(d2d * 0.30))
					local st2 = 0
					local pi2 = 0.09+math.random(0,4)/100
					local ms2 = math.random(3,7)
					for i=1,steps do
						...
						NoclipStep()
						if i==ms2 then task.wait(0.04+math.random(0,4)/100) ms2=i+math.random(4,9) end
						local s=AIEnabled and GetAIStepTime() or (0.007+math.random(0,2)/1000)
						st2=st2+s
						if st2>=pi2 then st2=0 pi2=0.09+math.random(0,4)/100 task.wait(0.032+math.random(0,3)/100)
						else task.wait(s) end
					end
					return true
				end
				...
				local function GetPrompt(target)
					if target.prompt and target.prompt.Parent then return target.prompt end
				...
				if #brainrots == 0 then
					...
					while AutoFarmEnabled and takenThisRun < CarryCount do
						if idx > #brainrots then
							brainrots = GetActiveBrainrots()
							idx = 1
							if #brainrots == 0 then task.wait(3) break end
						end
						local target = brainrots[idx]
						idx = idx + 1
						-- 5 attempts same as Trial
						local picked = false
						for attempt = 1, 5 do
							if not AutoFarmEnabled then break end
							if not target.obj or not target.obj.Parent then
								...
								for _,fb in ipairs(fresh) do
									if fb.rarity==target.rarity and fb.obj and fb.obj.Parent then
										target=fb found=true break
									end
								end
								if not found then task.wait(0.8) continue end
							end
							local h2=GetHRP() if not h2 then break end
							local arrived=FlyTo(h2,Vector3.new(target.pos.X,SAFE_Y,target.pos.Z))
							if not arrived then break end
							task.wait(0.1)
							if not target.obj or not target.obj.Parent then task.wait(0.5) continue end
						...
						if picked then
							...
							if takenThisRun < CarryCount then
								local delay=AIEnabled and GetAIDelay() or (FarmDelay+math.random(2,8)/10)
...
local TrialStatusHdr=MakeTxt({Size=UDim2.new(1,-14,0,12),Position=UDim2.new(0,12,0,4),Text=T("trialStatus"),TextColor3=Color3.fromRGB(50,50,50),TextSize=9,Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=8},TrialStatusCard)
local TrialStatusVal=MakeTxt({Size=UDim2.new(1,-14,0,18),Position=UDim2.new(0,12,0,22),Text=T("trialOff"),TextColor3=Color3.fromRGB(120,120,120),TextSize=12,Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=8},TrialStatusCard)
...
local TrialTaskHdr=MakeTxt({Size=UDim2.new(1,-14,0,12),Position=UDim2.new(0,12,0,4),Text=T("trialTask"),TextColor3=Color3.fromRGB(50,50,50),TextSize=9,Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=8},TrialTaskCard)
...
local TrialProgressHdr=MakeTxt({Size=UDim2.new(1,-14,0,12),Position=UDim2.new(0,12,0,4),Text=T("trialProgress"),TextColor3=Color3.fromRGB(50,50,50),TextSize=9,Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=8},TrialProgressCard)
...
local cfgSpeedVal=MakeTxt({Size=UDim2.new(0.2,0,0,22),Position=UDim2.new(0.35,0,0,44),Text="Ultra",TextColor3=Color3.fromRGB(185,185,185),TextSize=9,Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Center,ZIndex=8},cfgCard)
...
local speedLevels = {{v=0.09,n="Ultra"},{v=0.18,n="Fast"},{v=0.36,n="Normal"},{v=0.68,n="Slow"}}
local speedIdx = 1
local function ApplySpeedLevel()
	if TrialCFG then TrialCFG.FlySpeed = speedLevels[speedIdx].v end
	cfgSpeedVal.Text = speedLevels[speedIdx].n
	-- visual feedback: dim label if at limits
	cfgSm.TextColor3 = speedIdx == 1              and Color3.fromRGB(60,60,60) or Color3.fromRGB(200,200,200)
	cfgSp.TextColor3 = speedIdx == #speedLevels   and Color3.fromRGB(60,60,60) or Color3.fromRGB(200,200,200)
end
...
local trialDescLbl=MakeTxt({Size=UDim2.new(1,-72,0,13),Position=UDim2.new(0,14,0,31),Text=T("trialDesc"),TextColor3=Color3.fromRGB(68,68,68),TextSize=10,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=8},trialToggleCard)
...
-- JSON file: persist trial task so we don't re-scan every iteration
local TRIAL_CACHE_FILE = "starhub_trial_cache.json"
local function SaveTrialJSON()
	pcall(function()
		if not writefile then return end
		writefile(TRIAL_CACHE_FILE, HS:JSONEncode({
			rarity = TrialCache.rarity,
			need   = TrialCache.need,
			done   = TrialCache.done,
		}))
	end)
end
local function LoadTrialJSON()
	local ok, data = pcall(function()
		if not readfile or not isfile then return nil end
		if not isfile(TRIAL_CACHE_FILE) then return nil end
		return HS:JSONDecode(readfile(TRIAL_CACHE_FILE))
	end)
	if ok and data and data.rarity then
		TrialCache.rarity   = data.rarity
		TrialCache.need     = data.need or 0
		TrialCache.done     = data.done or 0
		TrialCache.lastRead = 0
		return data
	end
	return nil
end

-- Rarities that use respawn-and-fly: respawn at spawn then fly to brainrot
local RESPAWN_RARITIES = {
...
local function ReadTrialTask()
	-- Try JSON file on first call if no in-memory cache
	if not TrialCache.rarity then LoadTrialJSON() end
...
local function TrialFlyTo(hrp, toPos)
	...
	local timer = 0
	local pauseInt = 0.04 + math.random(0,2)/100
	...
	for i = 1, steps do
		...
		if timer >= pauseInt then
			timer=0 pauseInt=0.04+math.random(0,2)/100
			task.wait(0.010+math.random(0,1)/100)
		else
			task.wait(st)
		end
	end
	...
	if AutoTrialEnabled then
		...
		task.spawn(function()
			while AutoTrialEnabled do
				TrialStatusVal.Text=T("trialGoTower")
				...
				if not rarity or not IsTrialActive() then
					TrialStatusVal.Text=T("trialStarting")
				...
				if not rarity then
					TrialStatusVal.Text=T("trialNotFound")
				...
				TrialTaskVal.Text = rarity.." - "..need..T("trialUnits")..rewardTxt
				TrialProgressVal.Text = tostring(done).." / "..tostring(need)
				TrialStatusVal.Text = T("trialFarming")..rarity.."..."
				TrialStatusVal.TextColor3=Color3.fromRGB(100,220,100)
				AddLog("Trial: "..rarity.." x"..need)

				-- Find deliver prompt (tower drop-off)
				local function FindDeliverPrompt()
					for _, obj in ipairs(game:GetService("Workspace"):GetDescendants()) do
						if obj:IsA("ProximityPrompt") then
							local at = (obj.ActionText or ""):lower()
							local ot = (obj.ObjectText or ""):lower()
							if at:find("deliver") or at:find("drop") or at:find("give") or at:find("submit") then
								return obj
							end
							if ot:find("tower") and at ~= "" then
								return obj
							end
						end
					end
				...
				local function GetBrainrotPrompt(target)
				...
				local function DoDeliver()
					hrp = GetHRP2()
					if not hrp then return false end
					TrialStatusVal.Text = T("trialDeliver")
					TrialFlyTo(hrp, TOWER_POS)
					task.wait(TrialCFG.WaitAtTower)

					-- Try up to 5 times to deliver
					for attempt = 1, 5 do
						local delPrompt = FindDeliverPrompt()
						if not delPrompt then
							-- Maybe need to stand closer to trigger
							hrp = GetHRP2()
							if hrp then
								hrp.CFrame = CFrame.new(TOWER_POS.X + math.random(-2,2), SAFE_Y, TOWER_POS.Z + math.random(-2,2))
							end
							task.wait(0.3)
							delPrompt = FindDeliverPrompt()
						end
						if delPrompt and delPrompt.Parent then
					...
					-- Fallback: fire any prompt near tower
					for _, obj in ipairs(game:GetService("Workspace"):GetDescendants()) do
						if obj:IsA("ProximityPrompt") then
							local part = obj.Parent
							local pos = part and part:IsA("BasePart") and part.Position
							if pos and (pos - TOWER_POS).Magnitude < 20 then
								FireTrialPrompt(obj)
								task.wait(0.4)
								return true
							end
				...
				while AutoTrialEnabled do
					...
					if not IsTrialActive() or (n2 > 0 and d2 >= n2) then
						TrialStatusVal.Text=T("trialDone")
						TrialStatusVal.TextColor3=Color3.fromRGB(80,255,120)
						AddLog("Trial done! "..d2.."/"..n2)
						TrialCache.rarity = nil
						if TrialCFG.AutoRestart then
							TrialStatusVal.Text=T("trialRestart")
					...
					-- Перечитываем задание каждую итерацию чтобы поймать смену рарити
					TrialCache.rarity = nil
					local newRarity, newDone, newNeed = ReadTrialTask()
					if newRarity and newRarity ~= TrialRarity then
						TrialRarity = newRarity
						TrialNeed   = newNeed
						TrialDone   = newDone
						local info = TRIAL_INFO[newRarity]
						local rewardTxt = info and (" -> "..info.reward) or ""
						TrialTaskVal.Text = newRarity.." - "..tostring(newNeed)..T("trialUnits")..rewardTxt
						TrialStatusVal.Text = T("trialFarming")..newRarity.."..."
						TrialStatusVal.TextColor3 = Color3.fromRGB(100,220,100)
						AddLog(T("trialTaskChanged")..newRarity.." x"..tostring(newNeed))
					end

					local brainrots = TrialGetBrainrots(TrialRarity)
					if #brainrots == 0 then
						TrialStatusVal.Text=T("trialNoItems")..TrialRarity..T("trialNoItemsSuffix")
						task.wait(3)
					else
						-- Pick ONE brainrot, deliver, repeat
						local target = brainrots[1]
						-- Try up to 5 brainrots if one despawns
						local picked = false
						for attempt = 1, math.min(5, #brainrots) do
							target = brainrots[attempt]
							if not target.obj or not target.obj.Parent then continue end
							hrp = GetHRP2()
							if not hrp then break end
							local arrived
							if RESPAWN_RARITIES[TrialRarity] then
								TrialStatusVal.Text = T("trialRespawn")..TrialRarity.."..."
								AddLog("Респаун: "..TrialRarity)
								LocalPlayer:LoadCharacter()
								local _t = 0
								repeat task.wait(0.1) _t=_t+0.1 until GetHRP2() or _t > 8
								task.wait(1.5)
								EnableNoclip()
								hrp = GetHRP2()
								if not hrp then arrived = false else
									TrialStatusVal.Text = T("trialFlyTo")..TrialRarity.."..."
									arrived = TrialFlyTo(hrp, target.pos)
								end
							else
								TrialStatusVal.Text = T("trialGoTo")..TrialRarity.."..."
								arrived = TrialFlyTo(hrp, target.pos)
							end
							if not arrived then break end
							task.wait(TrialCFG.WaitAfterTake)
							-- Check still exists
							if not target.obj or not target.obj.Parent then
								AddLog(T("trialGone")..attempt.."/5)")
								continue
							end
							local prompt = GetBrainrotPrompt(target)
							if prompt and prompt.Parent then
								TrialStatusVal.Text = T("trialTaking")..TrialRarity.."..."
						...
						if picked then
							TrialCache.done = (TrialCache.done or 0) + 1
							SaveTrialJSON()
							DoDeliver()
							TrialCache.rarity = nil
							task.wait(6.3)
		...
		TrialStatusVal.Text=T("trialOff")
...
local function SelectLang(lang)
	...
	trialDescLbl.Text=T("trialDesc")
	TrialStatusHdr.Text=T("trialStatus")
	TrialTaskHdr.Text=T("trialTask")
	TrialProgressHdr.Text=T("trialProgress")
	TrialStatusVal.Text=T("trialOff")
