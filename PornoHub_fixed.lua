...
local IPAvatarTitle = Instance.new("TextLabel", InfoPanel)
IPAvatarTitle.Size = UDim2.new(1,-20,0,16); IPAvatarTitle.Position = UDim2.new(0,10,0,138)
IPAvatarTitle.BackgroundTransparency = 1; IPAvatarTitle.Text = "PLAYERS ON SERVER"
IPAvatarTitle.Font = Enum.Font.GothamBold; IPAvatarTitle.TextSize = 9
IPAvatarTitle.TextColor3 = PH_GREY
IPAvatarTitle.TextXAlignment = Enum.TextXAlignment.Left; IPAvatarTitle.ZIndex = 61

local IPAvatarScroll = Instance.new("ScrollingFrame", InfoPanel)
IPAvatarScroll.Size = UDim2.new(1,-20,0,80); IPAvatarScroll.Position = UDim2.new(0,10,0,142)
IPAvatarScroll.BackgroundColor3 = PH_DARK; IPAvatarScroll.BorderSizePixel = 0
IPAvatarScroll.ScrollBarThickness = 3; IPAvatarScroll.ScrollBarImageColor3 = PH_ORANGE
IPAvatarScroll.AutomaticCanvasSize = Enum.AutomaticSize.X
IPAvatarScroll.ScrollingDirection = Enum.ScrollingDirection.X
IPAvatarScroll.CanvasSize = UDim2.new(0,0,0,0); IPAvatarScroll.ZIndex = 61
Instance.new("UICorner", IPAvatarScroll).CornerRadius = UDim.new(0,8)
local IPGrid = Instance.new("UIListLayout", IPAvatarScroll)
IPGrid.FillDirection = Enum.FillDirection.Horizontal
IPGrid.Padding = UDim.new(0,5)
IPGrid.SortOrder = Enum.SortOrder.LayoutOrder
IPGrid.VerticalAlignment = Enum.VerticalAlignment.Center
local IPGridPad = Instance.new("UIPadding", IPAvatarScroll)
IPGridPad.PaddingTop=UDim.new(0,6); IPGridPad.PaddingLeft=UDim.new(0,6)
IPGridPad.PaddingRight=UDim.new(0,6); IPGridPad.PaddingBottom=UDim.new(0,6)

local IPNoAvatars = Instance.new("TextLabel", IPAvatarScroll)
IPNoAvatars.Size = UDim2.new(1,0,0,30); IPNoAvatars.BackgroundTransparency = 1
IPNoAvatars.Text = "Loading avatars..."; IPNoAvatars.Font = Enum.Font.Gotham
IPNoAvatars.TextSize = 11; IPNoAvatars.TextColor3 = Color3.fromRGB(55,55,55)
IPNoAvatars.Visible = true; IPNoAvatars.ZIndex = 62
...
local function LoadAvatars(tokens)
    -- Clear old
    for _, c in ipairs(IPAvatarScroll:GetChildren()) do
        if c:IsA("Frame") or c:IsA("ImageLabel") then c:Destroy() end
    end
    IPNoAvatars.Visible = true
    IPNoAvatars.Text = "Loading avatars..."

    if not tokens or #tokens == 0 then
        IPNoAvatars.Text = "No player data available."
        return
    end

    task.spawn(function()
        -- Build batch POST body
        local batch = {}
        for i, tok in ipairs(tokens) do
            table.insert(batch, {
                requestId = tostring(i-1),
                token = tok,
                type = "AvatarHeadShot",
                size = "60x60",
                format = "Png",
        ...
        if not ok or not res or res.StatusCode ~= 200 then
            IPNoAvatars.Text = "Could not load avatars."
            return
        end
        local data = lDecode(res.Body)
        if not data or not data.data then
            IPNoAvatars.Text = "No avatar data."
            return
        end
        IPNoAvatars.Visible = false
        for idx, item in ipairs(data.data) do
            local imgUrl = item.imageUrl or ""
            local cell = Instance.new("Frame", IPAvatarScroll)
            cell.Size = UDim2.fromOffset(60,68); cell.BackgroundColor3 = PH_CARD
            cell.BorderSizePixel = 0; cell.LayoutOrder = idx; cell.ZIndex = 62
            Instance.new("UICorner", cell).CornerRadius = UDim.new(0,7)
            local img = Instance.new("ImageLabel", cell)
            img.Size = UDim2.fromOffset(46,46); img.Position = UDim2.new(0.5,-23,0,4)
            img.BackgroundColor3 = PH_DARK; img.BorderSizePixel = 0
            img.Image = imgUrl; img.ZIndex = 63
            Instance.new("UICorner", img).CornerRadius = UDim.new(0,6)
            local numL = Instance.new("TextLabel", cell)
            numL.Size = UDim2.new(1,0,0,14); numL.Position = UDim2.new(0,0,0,52)
            numL.BackgroundTransparency = 1
            numL.Text = "#"..tostring(idx)
            numL.Font = Enum.Font.Gotham; numL.TextSize = 9
            numL.TextColor3 = PH_GREY; numL.ZIndex = 63
        end
    end)
end

local function ShowServerInfo(jobId, playing, maxP, ping, fps, tokens)
    ...
    IPStatPing.Text = ping and (tostring(ping).."ms") or "?ms"
    IPStatPing.TextColor3 = pingC
...
if isMobile then
    ...
    local function AddEntry(jobId, playing, maxP, ping, fps, tokens)
        ...
        table.insert(foundServers, {jobId=jobId, playing=playing, maxP=maxP, ping=ping or 9999, fps=fps, tokens=tokens})
        ...
        ib.MouseButton1Click:Connect(function()
            ShowServerInfo(jobId, playing, maxP, ping, fps, tokens)
        end)
